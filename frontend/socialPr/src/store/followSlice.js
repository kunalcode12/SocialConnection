...
export const getFollowers = (userId) => async (dispatch) => {
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:3000/api/v1/users/${userId}/followersAndFollowAndUnfollow`,
      {
        method: "GET",
        headers: {
...
export const getFollowing = (userId) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const response = await fetch(
      `http://127.0.0.1:3000/api/v1/users/${userId}/following`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
...
export const unfollowUser = (userId, user) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const response = await fetch(
      `http://127.0.0.1:3000/api/v1/users/${userId}/unfollow`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
